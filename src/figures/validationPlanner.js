...
const SATS = {
  iceye: {
    name: 'ICEYE', band: 'X-band', sub: '500 km · ~1.1 m antenna · 4 kW',
...
export default class ValidationPlanner extends Canvas2DFigure {
  ...
  draw() {
    ...
    const nrw = w < 480; // narrow mode: compact header (no sub-line), plot keeps most of the canvas
    const fs = nrw ? 11 : 13; // comparison-line font size
    const yName = nrw ? 22 : 26, yRes = nrw ? 42 : 66, yNesz = nrw ? 58 : 86;

    // header
    g.textAlign = 'left'; g.fillStyle = c.ink; g.font = `700 ${nrw ? 15 : 17}px ${FONT}`;
    g.fillText(S.name, 16, yName);
    const nameW = g.measureText(S.name).width; // measured with the name font
    g.fillStyle = c.echoCol; g.font = `600 ${nrw ? 10 : 12}px ${FONT}`;
    g.fillText(S.band, 16 + nameW + 12, yName);
    if (!nrw) { g.fillStyle = c.muted; g.font = `11px ${FONT}`; g.fillText(S.sub, 16, 44); }
    ...
    g.font = `${nrw ? 10 : 12}px ${FONT}`;
    g.fillStyle = c.muted; g.textAlign = 'right';
    g.fillText('published → model', w - 16, yName);
    g.textAlign = 'left';
    g.fillStyle = c.ink; g.font = `${fs}px ${FONT}`;
    g.fillText(`Range res ${S.pub.res.toFixed(1)} m → `, 16, yRes);
    let x = 16 + g.measureText(`Range res ${S.pub.res.toFixed(1)} m → `).width;
    g.fillStyle = c.echoCol; g.font = `600 ${fs}px ${FONT}`; g.fillText(fmtM(modelRes), x, yRes);
    x += g.measureText(fmtM(modelRes)).width + 6;
    g.fillStyle = resOk ? c.goodCol : c.badCol; g.font = `700 ${fs}px ${FONT}`; g.fillText(resOk ? '✓' : '≈', x, yRes);

    g.fillStyle = c.ink; g.font = `${fs}px ${FONT}`;
    const nzLabel = `NESZ ${S.pub.neszCmp} ${S.pub.nesz} dB → `;
    g.fillText(nzLabel, 16, yNesz);
    x = 16 + g.measureText(nzLabel).width;
    g.fillStyle = c.echoCol; g.font = `600 ${fs}px ${FONT}`; g.fillText(`${modelNesz.toFixed(1)} dB`, x, yNesz);
    x += g.measureText(`${modelNesz.toFixed(1)} dB`).width + 6;
    g.fillStyle = neszOk ? c.goodCol : c.badCol; g.font = `700 ${fs}px ${FONT}`; g.fillText(neszOk ? '✓' : '≈', x, yNesz);

    // SNR vs bandwidth curve
    const padL = nrw ? 36 : 44, padR = nrw ? 12 : 16, padT = nrw ? 66 : 100, padB = nrw ? 24 : 28;
    ...
    for (const bw of [10, 50, 100, 200, 300]) g.fillText(`${bw}`, lx(bw), y1 + (nrw ? 11 : 14));
    g.fillText('bandwidth (MHz)', x0 + plotW / 2, y1 + (nrw ? 21 : 26));
